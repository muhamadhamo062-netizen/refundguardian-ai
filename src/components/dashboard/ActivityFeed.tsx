...
export function ActivityFeed({ items }: ActivityFeedProps) {
  const defaultItems: ActivityItem[] = [
    { id: '1', type: 'scan', title: 'Scanning receipts...', time: 'Just now' },
    { id: '2', type: 'check', title: 'Checking delivery times...', time: '1m ago' },
    { id: '3', type: 'claim', title: 'Submitting refund claim...', time: '5m ago' },
    { id: '4', type: 'refund', title: 'Refund confirmed.', time: '12m ago' },
  ];
  ...
  return (
    <div className="rounded-xl border border-[var(--border)] bg-gradient-to-b from-[var(--card)] to-[#0d0e12] shadow-lg shadow-black/20">
      <div className="border-b border-[var(--border)] px-5 py-4 sm:px-6">
        <h3 className="text-sm font-semibold text-white">Real-time AI Activity</h3>
        <p className="mt-0.5 text-xs text-[var(--muted)]">
          Live feed of scans, claims, and refunds
        </p>
      </div>
      <ul className="divide-y divide-[var(--border)]/50">
        {displayItems.map((item, index) => (
          <li
            key={item.id}
            className={`flex gap-3 px-5 py-3.5 sm:px-6 ${index === 0 ? 'bg-white/[0.02]' : ''}`}
          >
            <span
